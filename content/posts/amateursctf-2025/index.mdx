...
又是個 server 題，加密邏輯是 flag 先被左移 256 bits 叫做 $F$，然後產生 100000 個隨機數 $r_i$，組成 $cs_i = F + r_i$。接著使用者可以輸入一個 值 $S$，程式會計算 $ms_i = (cs_i + S) unc n$，然後從這些 $ms_i$ 中隨機選一個 $m$ 進行 RSA 加密，輸出密文 $c = m^e unc n$，其中 $e = 3$。
...
$$ (s*n - a^n s*{n-1}) (a^{n+1} - 1) \equiv (s\_{n+1} - a^{n+1} s_n) (a^n - 1) \mod m $$
